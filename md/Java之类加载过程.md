...
#### 在虚拟机中，如何确定一个类？
在虚拟机运行时，类或接口不仅仅是有它的类的名称来决定，而是有一个值对：二进制名称和它的**定义加载器**共同确定的。形如&lt;$N$, $L_d$&gt;，其中 N 是类或者接口的名称，$L_d$ 是该类的定义加载器。就是说**运行时的一个类由类的全限定名和该类的定义加载器确定，以确定类的唯一性**。
...
2. **用来确定一个类**
    ...
    ```
#### 双亲委派机制
如果一个类的加载器$L1$收到类的加载请求，$L1$**不会马上加载**这个类，而是把这个请求委派给父类加载器，**每一个层次的类加载器都是如此**。因此，所有的加载请求都会最终委派到**启动类加载器**。只有当父类加载器表示无法完成这个加载请求时（通过抛出ClassNotFoundException异常提醒），子加载器才会**尝试**自己去加载。
...
- 负责加载**JAVA_HOME/lin/ext 目录中，或者被java.ext.dirs系统变量指定的路径中**的所有类库
- **可以直接使用**拓展类加载器
...
#### 各个加载器之间的层次关系
![](https://note.youdao.com/yws/api/personal/file/WEB21a7cc3fdf0a30a6f1ac5afcb97a1324?method=download&shareKey=f9af05fa502b66401d173f8a1b294e64)
