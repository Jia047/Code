...
##### 线程共享
###### 堆
- 内存区域中，最大的一块。用来存放对象实例，几乎所有的**对象实例**和**数组**的都是在这里分配内存的。（为什么说几乎？因为对于hotspot虚拟机而言，class对象虽然是对象，但是却存放在方法区中。）
...
- PC**随线程的诞生而创建**，这个位置内存很小（但很重要），大小是**一个字长**（64位系统的字长为 64bit），这样在**PC内就可以存储一个本地引用，也可以存储returnAdress**。
...
- **为什么要分老年代和新生代**？既然我们在说GC，那这样分肯定是帮我们在GC时简化问题。怎么简化？目前主流的垃圾收集器都使用**分代回收法**。对于不同的区，存放的对象有所区别，GC的方法也相应的有所不同。
...
> 内存分出去给对象了，JVM要怎样保证回收了这部分内存，不会搞崩系统？即判断对象的死活，只回收该回收的内存。
##### 判断一个对象是否该被回收
...
- 可以作为GC Roots的对象有哪些？
    ...
    - 方法区中**常量引用**的对象
- 枚举 GC Roots
在使用可达性分析算法列举$GC Roots$时，我们需要保证列举时对象的引用关
系是不会变化的（在列举的这个时刻不变）。因此，除了GC 线程外，其他的线程都需要停下来（Sun将这种情况称为"$Stop The World$"）。
但是，虚拟机应当准确地将无引用的对象找出来。那么，虚拟机是怎样知道哪些位置存在对象引用的呢？
虚拟机中用OopMap（$Ordinary Object Pointer$）记录了栈上本地变量到堆上对象的引用关系，这样就可以知道哪些位置是引用类型，就不必将整个栈的变量表扫描一遍。避免时间和空间上的浪费。
OopMap 的生成
OopMap使用来记录哪些位置是引用的数据结构，但是虚拟机中引用的关系时刻都在变化着，我们是否要时刻生成OopMap来记录这些引用关系呢？答案是不需要。只有在我们称之为“安全点（Safe Point）”的位置，我们才生成OopMap。
关于安全点和安全区域，这里有篇文章讲得很清楚：[传送门](http://wuzhangyang.com/2019/01/18/understand-safe-point/)
##### 回收方法区
###### 废弃常量的回收
...
- 该算法有以下特点
    ...
    - **空间利用率不高**，每次只能用可用内存的一半。代价太高，谁受得了。
> 复制算法对空间浪费实在太严重了。
###### 标记-整理算法
 - 标记：使用可达性分析算法，从根节点遍历，标记出要回收的对象。
...
- 该算法有以下特点
    - 分配内存时**不用考虑内存碎片**的问题。
    - 需要**移动大量对象**，处理效率不高。
> 上面的算法自身都是有好有坏，只适合于某种情况。而我们前面谈到给堆的分代是为了简化问题，方便GC。怎么个方便法呢？通过下面这个算法，你应该可以理解分代的好处。
###### 分代收集算法
